...
const txFixturesSip155 = require('./ttTransactionTestSip155VitaliksTests.json')
tape('[Transaction]: Basic functions', function (t) {
  ...
  t.test('should serialize', function (st) {
    transactions.forEach(function (tx) {
      st.deepEqual(tx.serialize(), srlp.encode(tx.raw))
    })
    st.end()
  })
  ...
  t.test('Verify SIP155 Signature based on Vitalik\'s tests', function (st) {
    txFixturesSip155.forEach(function (tx) {
      var pt = new Transaction(tx.srlp)
      st.equal(pt.hash(false).toString('hex'), tx.hash)
      st.equal('0x' + pt.serialize().toString('hex'), tx.srlp)
